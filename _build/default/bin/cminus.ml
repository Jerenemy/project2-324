...
let usage = 
  "dune exec cminus parse [opts] e:  parse e\n" ^
  "dune exec cminus exec p:  execute program p (path to file)\n"
...
let parse_and_show (cmd : parse_cmd_t) (s : string) : unit =
    ...
    | Pgm -> parse_and_show' Parser.terminated_pgm Ast.Prog.show s
    | PgmFile -> 
  ...
  try
    In_channel.with_open_text s (fun inch ->
      ...
      try
        ...
        let () = Interp.Io.show_prompts := true in
        Interp.exec p
      with
      | Parser.Error ->
        let pos = Lexing.lexeme_start_p lexbuf in
